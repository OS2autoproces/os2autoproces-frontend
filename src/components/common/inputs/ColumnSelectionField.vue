  ...
  <div>
    ...
    >
      <template v-slot:selection="{ item, index }">
        <span v-if="index === 0" class="placeholder-text">{{ placeholder }}</span>
      </template>
      <template slot="item" slot-scope="data">
        <SelectionFieldText :itemText="data.item[itemText]" :subText="data.item[itemSubText]" />
        <SelectionFieldAction v-if="hasAction" :actionIcon="actionIcon" @action="action(data.item)" />
    ...
    <div class="selection-text" :class="{ 'has-error': hasError }" v-if="disabled && value">{{ label }}</div>
  </div>
</template>
...
export default class ColumnSelectionField<T extends any> extends Vue {
  searchQuery = '';
